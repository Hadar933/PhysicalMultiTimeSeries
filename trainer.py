...
class Trainer:
	...
	def __init__(self,
				 features, targets, train_percent, val_percent, feature_lag, target_lag, intersect, batch_size,
	...
	def _set_loaders(self):
		...
		data_dict = utils.train_val_test_split(self.features, self.targets, self.train_percent, self.val_percent,
											   self.feature_lag, self.target_lag, self.intersect, self.batch_size)
		train_statistics = pd.DataFrame(data_dict['train']['data'].targets).describe()
		utils.update_yaml(self.yaml_path, train_statistics)
	...
	def _train_one_epoch(self, epoch: int) -> float:
		...
		for inputs, targets in tqdm_loader:
			...
			loss = 0.0
			for pred, tar in zip(predictions, targets): # multi output support
				loss = loss + self.criterion(pred, tar)
			total_loss += loss
			loss.backward()
			self.optimizer.step()
