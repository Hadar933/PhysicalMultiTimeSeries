...
def load_data_from_prssm_paper(path: str = "G:\\My Drive\\Master\\Lab\\flapping-wing-aerodynamics-prssm\\Data\\Input\\"
	...
	* beware - this is normalized w.r.t an unknown training data
	...
	if not targets_to_take: targets_to_take = [0, 1, 2, 3, 4]  # takes all
	kinematics = torch.Tensor(mat[kinematics_key])
	forces = torch.Tensor(mat[forces_key])
	if not targets_to_take: forces = forces[:, :, 0]
	return kinematics, forces


def train_val_test_split(kinematics: np.ndarray, forces: np.ndarray,
						 train_percent: float = 0.90, val_percent: float = 0.09,
						 feature_lag: int = 120, target_lag: int = 0, intersect: int = 0,
	...
	# using its index.
	all_test_datasets = {i: MultiTimeSeries(
	...
	all_test_dataloaders = {i: torch.utils.data.DataLoader(
		all_test_datasets[i],
		batch_size=1,
		shuffle=False
